...
/**
 * Desc:canal客户端程序,canal是cs架构,server端不需要编写代码,直接部署到Linux服务器即可
 *      客户端程序需要开发,连接服务器,与canal服务端进行交互
 **/
...
//                    printSummary(message);
                     ...
                     * 2.二进制字节码数据
                     */
                    //因为存储到kafka的数据是json格式的话,需要占用太多的存储资源,需要将存储到的数据格式化成占用比较小的数据存储格式
                    //1.将message对象转换成protobuf的字节码数据
                    byte[] bytes = binlogToProtoBuf(message);
                    //2.将binlog数据写入到kafka集群中
    ...
    private static byte[] binlogToProtoBuf(Message message) throws InvalidProtocolBufferException {
        ...
        com.canal_demo.CanalModel.RowData.Builder rowDataBuilder = com.canal_demo.CanalModel.RowData.newBuilder();
...
//        for (CanalEntry.Entry entry : message.getEntries()) {
//            // 事务开始,对于事务的开始和结束,不需要进行处理,处理的是数据本身
//            if (entry.getEntryType() == CanalEntry.EntryType.TRANSACTIONBEGIN
//                    || entry.getEntryType() == CanalEntry.EntryType.TRANSACTIONEND) {
//                continue;
//            }
